...
namespace sbbdep
{
  ...
  template <typename T> BackgroundJob<T>::~BackgroundJob () { stop (); }
  //------------------------------------------------------------------------------
  ...
  BackgroundJob<T>::push (const std::vector<T>& vals)
  {
    ...
    if (_queue.running)
      {
        LockGuard lock (_queue.mtx);
        _queue.messages.insert (
            _queue.messages.end (), vals.begin (), vals.end ());
        _queue.newData = true;
        _queue.condt.notify_one ();
        lock.unlock ();
        retval = true;
  ...
  auto
  BackgroundJob<T>::runBlocked (FT f) -> decltype (f ())
  {
    LockGuard lock (_queue.mtx);
    return f ();
